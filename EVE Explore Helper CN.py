...
checkbox = tk.Checkbutton(root, text="显示有星门星系", variable=show_stargate)
checkbox.grid(row=2, column=2, padx=20, pady=20, sticky="w")
show_visited = tk.BooleanVar()
checkbox = tk.Checkbutton(root, text="显示已访问星系", variable=show_visited)
...
def get_entry_data():
    ...
    if is_valid_name(name, name_list_all):
        ...
        messagebox.showwarning("警告", "不存在该星系!")


btn = tk.Button(root, text="搜索星系", command=get_entry_data)
...
btn = tk.Button(root, text="确认探索", command=confirm_explore)
